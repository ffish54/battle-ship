...
function updateInfo() {
  if (currentShip < ships.length) {
    shipDisplay.textContent = `${ships[currentShip].name} (${ships[currentShip].size})`;
  }

  shipCountDisplay.textContent = `${currentShip} / ${ships.length}`;
}

function generateEnemyFleet() {
    boardToCheck = enemyBoard;
  ships.forEach((ship) => {
    let placed = false;
    ...
    while (!placed) {
      ...
      if (canPlaceShip(enemyBoard, row, col, ship.size, randomDirection)) {
        placeShipEnemy(row, col, ship.size, randomDirection);
